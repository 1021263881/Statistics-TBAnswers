...
public class Tools
{
	...
	public class TB
	{
		...
		public JSONArray getpage(String cookie, String tid, int pn, int mpn, Boolean daoxu) throws mException
		{
			...
			try {
				...
				pagemax = Integer.valueOf(page.getJSONObject("page").getString("total_page"));
				return page.getJSONArray("post_list");
			} catch (JSONException e) {
				throw new mException("JSON解析好像遇到问题了", "JSON解析错误, Str:“" + thing + "”, 错误信息:" + e.toString());
			}
		}
		public ArrayList<String> getfloor(String cookie, String tid, String pid, int pn) throws mException
        {
			ArrayList<String> list = new ArrayList<String>();
		...
		public ArrayMap<Integer, ArrayMap<String, String>> anaPage(JSONArray list){
			int in = list.length();
			ArrayMap<Integer, ArrayMap<String, String>> tz = new ArrayMap<Integer, ArrayMap<String, String>>();
			if(in < 1){
				return tz;
			}
			for(int i = 0;i < in; i++){
